...
public class Matrix<T extends Number> implements WritableComparable<Matrix<T>> {
   ...
   * existe.
   */
  public static final Map<Integer, DataType> DataTypeID = createDataTypeID();

  /**
   * Initialisation de DataTypeID
   * 
   * @return DataTypeID
   */
  private static final Map<Integer, DataType> createDataTypeID() {
    Map<Integer, DataType> map = new HashMap<Integer, DataType>();
    map.put(DataType.Byte.typeID, DataType.Byte);
    map.put(DataType.Short.typeID, DataType.Short);
    map.put(DataType.Integer.typeID, DataType.Integer);
    map.put(DataType.Long.typeID, DataType.Long);
    map.put(DataType.Float.typeID, DataType.Float);
    map.put(DataType.Double.typeID, DataType.Double);
    map.put(DataType.BigInteger.typeID, DataType.BigInteger);
    map.put(DataType.BigDecimal.typeID, DataType.BigDecimal);
    return map;
  }
   ...
   */
  public static final Map<String, DataType> DataTypeDescriptionID =
      createDataTypeDescriptionID();

  /**
   * Initialisation de DataTypeDescriptionID
   * 
   * @return DataTypeDescriptionID
   */
  private static final Map<String, DataType> createDataTypeDescriptionID() {
    Map<String, DataType> map = new HashMap<String, DataType>();
    map.put(DataType.Byte.descriptionID, DataType.Byte);
    map.put(DataType.Short.descriptionID, DataType.Short);
    map.put(DataType.Integer.descriptionID, DataType.Integer);
    map.put(DataType.Long.descriptionID, DataType.Long);
    map.put(DataType.Float.descriptionID, DataType.Float);
    map.put(DataType.Double.descriptionID, DataType.Double);
    map.put(DataType.BigInteger.descriptionID, DataType.BigInteger);
    map.put(DataType.BigDecimal.descriptionID, DataType.BigDecimal);
    return map;
  }
   ...
   */
  private FunctionWithException<DataInput, T, IOException> readDataFunction =
      null;
   ...
   * @throw     {@link java.io.IOException}
   */
  private BiConsumerWithException<DataOutput, T,
      IOException> writeDataFunction = null;
   ...
   * @throw    {@link java.io.IOException}
   */
  private BiConsumerWithException<DataInput, Integer,
      IOException> skipDataFunction = null;
   ...
   */
  public FunctionWithException<DataInput, T, IOException>
      getReadDataFunction() {
   ...
   */
  public BiConsumerWithException<DataOutput, T, IOException>
   ...
   *                          dans un flux
   */
  public void setWriteDataFunction(final BiConsumerWithException<DataOutput, T,
      IOException> writeDataFunction) {
   ...
   */
  public BiConsumerWithException<DataInput, Integer, IOException>
      getSkipDataFunction() {
   ...
   */
  public void setSkipDataFunction(BiConsumerWithException<DataInput, Integer,
      IOException> skipDataFunction) {
   ...
   *                                {@code true}
   * @param  random                 des valeurs aleatoires seront affectees a
   *                                tous les elements d'une matrice, si
   *                                {@code alloc} est {@code true} et si
   *                                {@code value} est {@code null}
   * @param  conf                   une configuration de Apache Hadoop
   * @param  f                      un nom de fichier ou ecrire tous les
   ...
   *                                matrice dans un flux
   * @param  skipDataFunction       la fonction de saut d'un nombre de byte dans
   *                                un flux
   * @param  randomDataFunction     la fonction generant un nombre alleatoire
   *                                correspondant au type des element d'une
   *                                matrice
   ...
   */
  private void init(final int n, final int m, final DataType dataType,
      final T ZERO, final boolean alloc, final String value,
      final boolean random, final Configuration conf, final Path f,
      final BiFunction<Integer, Integer, T[][]> newDataFunction,
      final FunctionWithException<DataInput, T, IOException> readDataFunction,
      final BiConsumerWithException<DataOutput, T,
          IOException> writeDataFunction,
      final BiConsumerWithException<DataInput, Integer,
          IOException> skipDataFunction,
      final Function<Random, T> randomDataFunction,
      final Function<String, T> convertionDataFunction) throws IOException {
    this.n = n;
    this.m = m;
    this.dataType = dataType;
    this.ZERO = ZERO;
   ...
   * @param  alloc       si {@code true} alors l'espace reserve aux elements
   *                     sera alloue, si {@code false} alors l'espace reserve
   *                     aux elements ne sera pas alloue
   * @throws IOException une exception I/O a ete relevee
   */
   ...
   * @throws IOException            une exception I/O a ete relevee
   */
  public Matrix(final int n, final int m, final DataType dataType, final T ZERO,
      final boolean alloc, final String value, final boolean random,
      final Configuration conf, final Path f,
      final BiFunction<Integer, Integer, T[][]> newDataFunction,
      final FunctionWithException<DataInput, T, IOException> readDataFunction,
      final BiConsumerWithException<DataOutput, T,
          IOException> writeDataFunction,
      ...
      final Function<Random, T> randomDataFunction,
      final Function<String, T> convertionDataFunction) throws IOException {
    init(n, m, dataType, ZERO, alloc, value, random, conf, f, newDataFunction,
        readDataFunction, writeDataFunction, skipDataFunction,
        randomDataFunction, convertionDataFunction);
  }
  ...
  /**
   * Instancie une nouvelle matrice en recuperant les valeurs des membres d'une
   * autre matrice {@code x} en precisant, par {@code alloc}, si un nouvel
   * espace, de dimensions differentes, par {@code n} et {@code m}, que
   ...
   * {@code x.dataType=DataType.Short} et si {@code y.dataType=DataType.Byte} ou
   * {@code y.dataType=DataType.Short} alors le type des elements de la nouvelle
   * matrice est {@code DataType.Integer},
   ...
   * {@code y.dataType=DataType.Float} ou {@code y.dataType=DataType.Double}
  ...
  public static Matrix<? extends Number> builder(final int n, final int m,
    ...
    DataType dataType =
        (x.dataType.typeID > y.dataType.typeID) ? x.dataType : y.dataType;
    ...
    if (x.dataType == DataType.BigInteger || y.dataType == DataType.BigInteger)
      if (x.dataType == DataType.Float || y.dataType == DataType.Float
          || x.dataType == DataType.Double || y.dataType == DataType.Double)
  ...
  /**
   * Construit une nouvelle instance d'une matrice aux dimensions {@code width}
   * et {@code height} et les autre membres seront determines a partir
   * {@code dataType}.
   *
   ...
   *                               alloue
   * @param  value                 une valeur qui sera affectee a tous les
   *                               elements de la nouvelle matrice, si alloc est
   *                               {@code true}
   * @param  random                des valeurs aleatoires seront affectes a tous
   *                               les elements de la nouvelle matrice, si alloc
    ...
    switch (dataType) {
      case Byte:
        x = new Matrix<Byte>(width, height, DataType.Byte, (byte) 0, alloc,
            ...
            (DataInput in) -> in.readByte(),
            (DataOutput out, final Byte a) -> out.writeByte(a), (DataInput in,
                final Integer toSkip) -> in.skipBytes(toSkip * Byte.BYTES),
            (Random r) -> {
              byte[] b = new byte[1];
              r.nextBytes(b);
              return Byte.valueOf(b[0]);
            }, (final String s) -> Byte.parseByte(s));
        break;
      case Short:
        x = new Matrix<Short>(width, height, DataType.Short, (short) 0, alloc,
            value, random, conf, f,
            (final Integer n, final Integer m) -> new Short[n][m],
            (DataInput in) -> in.readShort(),
            (DataOutput out, final Short a) -> out.writeShort(a),
            (DataInput in, final Integer toSkip) -> in
                .skipBytes(toSkip * Short.BYTES),
            (Random r) -> Short
                .valueOf(Integer.valueOf(r.nextInt()).shortValue()),
            (final String s) -> Short.parseShort(s));
        break;
      case Integer:
        x = new Matrix<Integer>(width, height, DataType.Integer, 0, alloc,
            value, random, conf, f,
            (final Integer n, final Integer m) -> new Integer[n][m],
            (DataInput in) -> in.readInt(),
            (DataOutput out, final Integer a) -> out.writeInt(a),
            (DataInput in, final Integer toSkip) -> in
                .skipBytes(toSkip * Integer.BYTES),
            (Random r) -> Integer.valueOf(r.nextInt()),
            (final String s) -> Integer.parseInt(s));
        break;
      case Long:
        x = new Matrix<Long>(width, height, DataType.Long, 0l, alloc, value,
            random, conf, f,
            (final Integer n, final Integer m) -> new Long[n][m],
            (DataInput in) -> in.readLong(),
            (DataOutput out, final Long a) -> out.writeLong(a),
            (DataInput in, final Integer toSkip) -> in
                .skipBytes(toSkip * Long.BYTES),
            (Random r) -> Long.valueOf(r.nextLong()),
            (final String s) -> Long.decode(s));
        break;
      case Float:
        x = new Matrix<Float>(width, height, DataType.Float, 0f, alloc, value,
            random, conf, f,
            (final Integer n, final Integer m) -> new Float[n][m],
            (DataInput in) -> in.readFloat(),
            (DataOutput out, final Float a) -> out.writeFloat(a),
            (DataInput in, final Integer toSkip) -> in
                .skipBytes(toSkip * Float.BYTES),
            (Random r) -> Float.valueOf(r.nextFloat()),
            (final String s) -> Float.parseFloat(s));
        break;
      case Double:
        x = new Matrix<Double>(width, height, DataType.Double, 0d, alloc, value,
            random, conf, f,
            (final Integer n, final Integer m) -> new Double[n][m],
            (DataInput in) -> in.readDouble(),
            (DataOutput out, final Double a) -> out.writeDouble(a),
            (DataInput in, final Integer toSkip) -> in
                .skipBytes(toSkip * Double.BYTES),
            (Random r) -> Double.valueOf(r.nextDouble()),
   ...
   *             {@link Matrice}
   */
  public static <T extends Number> Map<BlockKey, Matrix<T>>
      toBlocks(final Matrix<T> x, final int nb, final int mb) {
   ...
   * @param  <U>                      le type des elements de la matrice
   *                                  destination
   * @param  dest                     la matrice destination
   * @param  src                      la matrice source
   * @param  srci                     la ligne de la matrice source ou commencer
   *                                  la copie
   * @param  srcj                     la colonne de la matrice source ou
   *                                  commencer la copie
   * @param  desti                    la ligne de la matrice destination ou
   *                                  commencer la copie
   * @param  destj                    la colonne de la matrice destination ou
   *                                  commencer la copie
   * @param  n                        le nombre de ligne a copier
   * @param  m                        le nombre de colonne a copier
   * @param  copyDataFunction         la fonction de copie permettant de
   *                                  transformer un element de type T en un
   *                                  element de type U
   * @throws MatrixBoundCopyException Cette exception est levee si
   ...
   */
  public static <T extends Number, U extends Number> void copy(Matrix<U> dest,
      final Matrix<T> src, final int srci, final int srcj, final int desti,
      final int destj, final int n, final int m,
    ...
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++)
        dest.data[desti + i][destj + j] =
            copyDataFunction.apply(src.data[srci + i][srcj + j]);
  }
   ...
   * @param  in          le flux ou lire
   * @param  alloc       si {@code true} alors l'espace reserve aux elements
   *                     sera alloue et les elements de la matrice seront lus,
   *                     si {@code false} alors l'espace reserve aux elements ne
   *                     sera pas alloue et les elments de la matrice ne seront
   *                     pas lus
   * @return             la matrice lue
   * @throws IOException une exception I/O a ete relevee
   */
  public static Matrix<? extends Number> read(DataInput in, final boolean alloc)
      throws IOException {
    int n = in.readInt(), m = in.readInt();
    DataType dataType = DataTypeID.get(in.readInt());
    Matrix<? extends Number> x =
        builder(n, m, dataType, alloc, null, false, null, null);
    if (alloc)
      readData(in, x);
    return x;
  }
   ...
   * @param  in          le flux ou lire
   * @param  x           la matrice ou les elements lu seront stockes. Il est
   *                     suppose que les membres, a part les elements, sont
   *                     renseignes.
   * @throws IOException une exception I/O a ete relevee
   */
   ...
   * @param  in                       le flux ou lire
   * @param  x                        une matrice correspondant aux donnees
   *                                  stockees dans le flux. La matrice peut
   *                                  etre vide d'element.
  ...
  public static <T extends Number> Matrix<T> readData(DataInput in,
      final Matrix<T> x, final int i0, final int j0, final int n, final int m)
      throws IOException, MatrixBoundReadException {
   ...
   *
   * @param  <T>                       le type des elements de la matrice a
   *                                   ecrire
   * @param  out                       le flux ou ecrire
   * @param  x                         la matrice contenant les elements a
   *                                   ecrire
   ...
   * @param  header                    si {@code true} alors l'entete de la
   *                                   matrice sera ecrit, si {@code false}
   *                                   alors l'entete ne sera pas ecrit
  ...
  public static <T extends Number> void writeData(DataOutput out,
      final Matrix<T> x, final int i0, final int j0, final int n, final int m,
      final boolean header) throws IOException, MatrixBoundWriteException {
    if ((i0 + n > x.n) || (j0 + m > x.m))
      throw new MatrixBoundWriteException();
    if (header) {
      out.writeInt(n);
   ...
   * @param  <R>                          le type des elements de la matrice
   *                                      resultante
   * @param  x                            un des termes de la somme
   * @param  y                            un des termes de la somme
   * @param  sumDataFunction              la fonction permettant de sommer un
   *                                      element de type {@code T} avec un
   *                                      element de type {@code U} et de
   *                                      retourner une element de type
   *                                      {@code R}
   * @param  virtual                      si {@code true} alors il est possible
   *                                      de sommer des matrices de dimensions
   *                                      differentes, si {@code false} alors si
   *                                      les dimensions des matrices different
   *                                      une exception de type
   *                                      MatrixBoundAdditionException est levee
   * @return                              une matrice resultant de la somme de
   *                                      deux matrices
   * @throws MatrixBoundAdditionException si le parametre {@code virtual} est
   *                                      {@code false} alors si les dimensions
   *                                      des matrices different une exception
   *                                      de type MatrixBoundAdditionException
   *                                      est levee
   */
  public static <T extends Number, U extends Number, R extends Number> Matrix<R>
      sum(final Matrix<T> x, final Matrix<U> y,
    ...
    for (int i = 0; i < n; i++)
      for (int j = 0; j < m; j++)
        r.data[i][j] =
            sumDataFunction.apply((i < x.n && j < x.m) ? x.data[i][j] : x.ZERO,
   ...
   * sont supposes etre de meme type. Les termes de l'addition peuvent ne pas
   * etre de memes dimensions, l'addition suppose alors que les elements absents
   * de la matrice la plus petite est zero.
   *
   * @param  <T>                          le type des elements des matrices
   * @param  x                            Une matrice ou sera ecrit les
   *                                      resultats de l'addition
   * @param  y                            Une matrice
   * @param  sumDataFunction              la fonction permettant d'additionner
   *                                      un element de type {@code T} avec un
   *                                      element de type {@code T} et de
   *                                      retourner une element de type
   *                                      {@code T}
   * @param  virtual                      si {@code true} alors il est possible
   *                                      d'additionner des matrices de
   *                                      dimensions differentes, si
   *                                      {@code false} alors si les dimensions
   *                                      des matrices different une exception
   *                                      de type MatrixBoundAdditionException
   *                                      est levee
   ...
   *
   * @param  <T>                                le type des elements d'un des
   *                                            termes
   * @param  <U>                                le type des elements d'un des
   *                                            termes
   * @param  <R>                                le type des elements de la
   *                                            matrice resultante
   * @param  x                                  un des termes de la
   *                                            multiplication
   * @param  y                                  un des termes de la
   *                                            multiplication
   * @param  sumDataFunction                    la fonction permettant
   *                                            d'additionner un element de type
   *                                            {@code R} avec un element de
   *                                            type {@code R} et de retourner
   *                                            une element de type {@code R}
   * @param  multiplyDataFunction               la fonction permettant de
   *                                            multiplier un element de type
   *                                            {@code T} avec un element de
   *                                            type {@code U} et de retourner
   *                                            une element de type {@code R}
   * @return                                    une matrice resultante d'une
   *                                            multiplication
   * @throws MatrixBoundMultiplicationException cette exception est levee si
   *                                            {@code x.getWidth()!=y.getHeight()}
   */
  public static <T extends Number, U extends Number, R extends Number> Matrix<R>
      multiply(final Matrix<T> x, final Matrix<U> y,
    ...
    for (int i = 0; i < x.n; i++)
      ...
      for (int j = 0; j < y.m; j++) {
        ...
        for (int k = 1; k < x.m; k++)
          a = sumDataFunction.apply(a,
              multiplyDataFunction.apply(x.data[i][k], y.data[k][j]));
        r.data[i][j] = a;
      }
    return r;
  }
   ...
   * partie d'une multiplication de deux matrices. Les termes de la
   * multiplication et son resultat sont integralement charges en memoire.
   *
   * @param <T> le type des elements d'un des termes
   * @param <U> le type des elements d'un des termes
   ...
   */
  private static class ConcurrentMultiplication<T extends Number,
      U extends Number, R extends Number> implements Runnable {
     ...
     * charge de calculer la multiplication la partition de la partition de la
     * matrice resultant qui lui est dediee.
     */
    @Override
    public void run() {
      int n, n1, m, m1;
      ...
      for (int i1 = 0; i1 < n1; i1++)
        ...
        for (int j1 = 0; j1 < m1; j1++) {
          R a = multiplyDataFunction.apply(x.data[(ithread * n) + i1][0],
              y.data[0][(jthread * m) + j1]);
          // Boucle sur les elements des partitions des matrices des
          // termes de la multiplication
          for (int k1 = 1; k1 < x.m; k1++)
            a = sumDataFunction.apply(a,
                multiplyDataFunction.apply(x.data[(ithread * n) + i1][k1],
  ...
  /**
   * Multiplication, parallele par multithreading, de deux matrices. Les termes
   * de la multiplication et son resultat sont integralement charges en memoire.
   *
   * @param  <T>                                le type des elements d'un des
   *                                            termes
   * @param  <U>                                le type des elements d'un des
   ...
   * @param  y                                  un des termes de la
   *                                            multiplication
   * @param  nthreads                           le nombre de thread qui est le
   *                                            nombre de partition sur les
   *                                            lignes de la matrice resultante
   * @param  mthreads                           le nombre de thread qui est le
   *                                            nombre de partition sur les
  ...
  @SuppressWarnings("unchecked")
  public static <T extends Number, U extends Number, R extends Number> Matrix<R>
      multiply(final Matrix<T> x, final Matrix<U> y, final int nthreads,
    ...
    if (np * mp == 1)
      ...
      for (int i = 0; i < np; i++)
        for (int j = 0; j < mp; j++)
          executor.execute(new ConcurrentMultiplication<T, U, R>(r, x, y, np,
              mp, i, j, sumDataFunction, multiplyDataFunction));
   ...
   * matrices. Une instance de la classe est associee a un thread executant une
   * partie d'une multiplication de deux matrices. Les termes de la
   * multiplication et son resultat sont stockes sur disque. Des partitions des
   * termes de la multiplication sont chargees en memoire en fonction de la
   * partition a calculer, et chargee en memoire, de la matrice resultante.
   *
   * @param <T> le type des elements d'un des termes
   * @param <U> le type des elements d'un des termes
   * @param <R> le type des elements de la matrice resultante
   */
  private static class ConcurrentBlocksMultiplication<T extends Number,
      U extends Number, R extends Number> implements Runnable {
    ...
    /**
     * {@code nthreads} et {@code mthreads} sont les nombres de partition,
     * respectivement sur les partitions sur les lignes et sur les partitions
     * sur les colonnes.
     */
    private final int nthreads, mthreads;

    /**
     * {@code nbsize} et {@code mbsize} sont respectivement le nombre de ligne
     * et le nombre de colonne d'une partition de la matrice resultante
     */
    ...
    /**
     * {@code nb} et {@code mb} sont les nombres de partition, respectivement
     * sur les lignes et sur les colonnes de la matrice resultante.
     */
     ...
     */
    @SuppressWarnings("unchecked")
    public ConcurrentBlocksMultiplication(final Path rPath, final Path xPath,
        final Path yPath, final Map<BlockKey, Matrix<T>> xBlocks,
        final Map<BlockKey, Matrix<U>> yBlocks, final int ithread,
        final int jthread, final int nbsize, final int mbsize, final int kbsize,
        final int nb, final int mb, final int nthreads, final int mthreads,
        ...
        for (int i = 0, xi = ithread * nb1; i < nb2; i++, xi++)
          ...
          for (int j = 0, xj = jthread * mb1; j < mb2; j++, xj++) {
            ...
            r = multiply(readData(xIn, x, xi * nbsize, 0, a.n, a.m),
                readData(yIn, y, 0, xj * mbsize, b.n, b.m), sumDataFunction,
            ...
            for (int k = 1; k < mb; k++) {
              ...
              s = multiply(readData(xIn, x, xi * nbsize, k * kbsize, a.n, a.m),
                  readData(yIn, y, k * kbsize, xj * mbsize, b.n, b.m),
            ...
            FSDataOutputStream out =
                FileSystem.get(conf).create(getPartitionPath(rPath, xi, xj));
            r.write(out);
            out.close();
          }
        xIn.close();
   ...
   * multiplication de deux matrices et sauvegarde la matrice jointe sur disque.
   *
   * @param  <T>                      le type des elements d'un des termes
   * @param  <U>                      le type des elements d'un des termes
   * @param  fs                       Le systeme de fichier ou les fichiers
   *                                  seront lu et ecrits
   * @param  f                        le chemin correspondant au prefix des
   *                                  fichiers des partions à joindre et du
   *                                  fichier ou la matrice resultante sera
   *                                  stocker
   * @param  x                        un des termes de la multiplication
   * @param  y                        un des termes de la multiplication
   * @param  delete                   si {@code true} alors les fichiers des
   *                                  partitions seront effaces apres la
   *                                  jointure
  ...
  public static <T extends Number, U extends Number> void joinBlocks(
      ...
      final int nb, final boolean delete)
      throws IOException, MatrixBoundReadException {
    ...
    writeHeader(out, r);
    for (int i = 0; i < nb; i++)
      for (int k = 0, nb1 = (i == nb - 1) ? (x.n - (i * nbsize)) : nbsize;
          k < nb1; k++)
        for (int j = 0; j < nb; j++) {
  ...
  /**
   * Multiplication, parallele par multithreading, de deux matrices. Les termes
   * de la multiplication et son resultat sont integralement stockes sur disque.
   *
   * @param  <T>                                le type des elements d'un des
   *                                            termes
   * @param  <U>                                le type des elements d'un des
   ...
   * @param  <R>                                le type des elements de la
   *                                            matrice resultante
   * @param  rPath                              le chemin correspondant au
   *                                            prefix des fichiers des partions
   *                                            à joindre et du fichier ou la
   *                                            matrice resultante sera stocker
   * @param  xPath                              le chemin correspondant au
   *                                            fichier d'une matrice des termes
   *                                            de la multiplication
   * @param  yPath                              le chemin correspondant au
   *                                            fichier d'une matrice des termes
   *                                            de la multiplication
   * @param  nb                                 nombre de partition sur les
   *                                            lignes de la matrice resultante
   * @param  mb                                 nombre de partition sur les
   *                                            colonnes de la matrice
   *                                            resultante
   * @param  nthreads                           le nombre de thread qui est le
   *                                            nombre de partition sur les
   *                                            partitions sur les lignes de la
   *                                            matrice resultante
   * @param  mthreads                           le nombre de thread qui est le
   *                                            nombre de partition sur les
   *                                            partitions sur les colonnes de
   *                                            la matrice resultante
   * @param  conf                               La configuration de
   *                                            l'environnement d'execution de
   *                                            Apache Hadoop. Cette
   *                                            configuration permet de
   *                                            recuperer des parametres pour
   *                                            effectuer des operations E/S.
   * @param  sumDataFunction                    la fonction permettant
   *                                            d'additionner un element de type
   *                                            {@code R} avec un element de
   *                                            type {@code R} et de retourner
   ...
   */
  public static <T extends Number, U extends Number, R extends Number> void
      multiply(final Path rPath, final Path xPath, final Path yPath,
          final int nb, final int mb, final int nthreads, final int mthreads,
          final Configuration conf, final BiFunction<R, R, R> sumDataFunction,
          final BiFunction<T, U, R> multiplyDataFunction)
          throws IOException, MatrixBoundMultiplicationException,
    ...
    if (np * mp > 1) {
      ...
      ExecutorService executor =
          Executors.newFixedThreadPool(nthreads * mthreads);
      ...
      for (int i = 0; i < nthreads; i++)
        for (int j = 0; j < mthreads; j++)
          executor.execute(new ConcurrentBlocksMultiplication<T, U, R>(rPath,
              xPath, yPath, xBlocks, yBlocks, i, j, nbsize, mbsize, kbsize, nb,
              mb, nthreads, mthreads, conf, sumDataFunction,
              multiplyDataFunction));
      // Demande au thread lance de se terminer quand leur traitement est
      // termine
      executor.shutdown();
  ...
  public static Path checkConfigurationName(FileSystem fs, Configuration conf,
      final String property, final Path prefix, final boolean input)
    ...
    if (name == null)
      ...
      f = fs.makeQualified(
          (prefix != null) ? new Path(prefix, name) : new Path(name));
   ...
   */
  @SuppressWarnings("unchecked")
  public static <T extends Number, U extends Number, R extends Number> int
      multiply(Configuration conf, final BiFunction<R, R, R> sumDataFunction,
          final BiFunction<T, U, R> multiplyDataFunction) throws IOException,
    ...
    int nthreads = conf.getInt(MatrixParameter.nThreads, 0),
        mthreads = conf.getInt(MatrixParameter.mThreads, 0);
    int result = 1;
    inputDir =
        checkConfigurationName(fs, conf, MatrixParameter.inputDir, null, true);
    ...
    outputDir = checkConfigurationName(fs, conf, MatrixParameter.outputDir,
        null, false);
    rPath = checkConfigurationName(fs, conf, MatrixParameter.rPrefixFilename,
        outputDir, false);
    xIn = fs.open(xPath);
    yIn = fs.open(yPath);
    x = (Matrix<T>) readHeader(xIn);
