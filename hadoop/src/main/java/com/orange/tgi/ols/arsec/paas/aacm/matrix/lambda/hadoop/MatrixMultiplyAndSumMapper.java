...
/** {@inheritDoc} */
public class MatrixMultiplyAndSumMapper<T extends Number, U extends Number,
    R extends Number>
  ...
  protected void setup(Context context)
    ...
    xIn = FileSystem.get(conf).open(new Path(conf.get(MatrixParameter.xPath)));
    yIn = FileSystem.get(conf).open(new Path(conf.get(MatrixParameter.yPath)));
  ...
  protected void map(BlockKey key, NullWritable value, Context context)
    ...
    try {
      ...
      r = Matrix.multiply(
          Matrix.readData(xIn, x, key.geti() * nbsize, 0, a.getHeight(),
              a.getWidth()),
          Matrix.readData(yIn, y, 0, key.getj() * mbsize, b.getHeight(),
              b.getWidth()),
          nthreads, mthreads, sumDataFunction, multiplyDataFunction);
      for (int k = 1; k < mb; k++) {
        ...
        yIn.seek(Matrix.HEADER_SIZE);
        s = Matrix.multiply(
            Matrix.readData(xIn, x, key.geti() * nbsize, k * kbsize,
                a.getHeight(), a.getWidth()),
            Matrix.readData(yIn, y, k * kbsize, key.getj() * mbsize,
                b.getHeight(), b.getWidth()),
            nthreads, mthreads, sumDataFunction, multiplyDataFunction);
        if ((r.getHeight() < s.getHeight()) || (r.getWidth() < s.getWidth())) {
